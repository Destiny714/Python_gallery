...
def bark_pusher(title,content):
    for barkid in bark_ids:
        url = f'https://api.day.app/{barkid}/{title}/{content}'
        requests.get(url)
...
if __name__ == '__main__':
    spu_dict = {451:'iphone 13 pro max', 420:'iphone 13 pro', 405:'iphone13'}
    print(spu_dict)
    spu = input('输入商品编号:')
    city = input('输入城市:')
    bark_id = input('输入推送id,多个用(,)隔开:')
    bark_ids = []
    for _ in bark_id.split(','):
        if _ != '':
            bark_ids.append(_)
